      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 ${direction === 'rtl' ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`${direction === 'rtl' ? 'md:order-3' : 'md:order-1'}`}>
            ...
            >
              {language === 'ar' 
                ? 'هيئة طلابية نقابية وطنية منتخبة ومستقلة، تهدف لتمثيل الحركة الطلابية السورية داخل الجامعات التركية، تستمد شرعيتها من قواعدهـا الطلابيـة الجامعيـة، و تمثل الطلبـة السوريين بقضاياهم وحقوقهم في تركيا.'
                : language === 'en'
